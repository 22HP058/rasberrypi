...
while True:
    ...
    #여기서 모델 작동 후 (위치 , label index , score) 갖고옴  (객체 detect 개수 )
        ...
        if ((scores[i] > min_conf_threshold) and (scores[i] <= 1.0)):
            ...
            if(object_name in USELABEL):
                ...
                if(object_name =="traffic light"):
                    ...
                    red = np.count_nonzero(maskRED ==255)
                    green = np.count_nonzero(maskGREEN ==255)
                    blue = np.count_nonzero(maskBLUE ==255)
